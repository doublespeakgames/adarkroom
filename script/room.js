...
var Room = {
  ...
  updateBuildButtons: function() {
    ...
    for (var k in Room.Craftables) {
      ...
      if (craftable.button == null) {
        if (Room.craftUnlocked(k)) {
          ...
          }).css('opacity', 0).attr('buildThing', k).appendTo(loc).animate({
            opacity: 1
          }, 300, 'linear');
        }
      } else {
    ...
    for (var g in Room.TradeGoods) {
      ...
      if (good.button == null) {
        if (Room.buyUnlocked(g)) {
          ...
          }).css('opacity', 0).attr('buildThing', g).appendTo(buySection).animate({
            opacity: 1
          }, 300, 'linear');
        }
      } else {
    ...
    if (needsAppend && buildSection.children().length > 0) {
      buildSection.appendTo('div#roomPanel').animate({
        opacity: 1
      }, 300, 'linear');
    }
    if (cNeedsAppend && craftSection.children().length > 0) {
      craftSection.appendTo('div#roomPanel').animate({
        opacity: 1
      }, 300, 'linear');
    }
    if (bNeedsAppend && buildSection.children().length > 0) {
      buySection.appendTo('div#roomPanel').animate({
        opacity: 1
      }, 300, 'linear');
    }
  },
